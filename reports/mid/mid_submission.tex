...
Wang et al.\ \shortcite{wang2022interpretabilitywildcircuitindirect}
established a framework for identifying \emph{circuits}—subsets of model
components responsible for specific behaviours. Their analysis of the
Indirect Object Identification (IOI) task revealed several functional
classes of attention heads, including \textbf{inhibitory heads} that
suppress competing tokens and redirect probability mass toward the
correct completion.
...
Each layer's MLP is:
\[ \mathrm{FFN}(\mathbf{x}) = \mathbf{W}_2\,\mathrm{GELU}(\mathbf{W}_1 \mathbf{x}). \]
A key interpretability insight is that FFNs behave like key--value memory
stores~\cite{geva-etal-2021-transformer}: mid-layer FFNs strongly store
entity--attribute associations.  In our framework:
\begin{itemize}
...
\label{sec:sgr}
For a negated prompt we define the SGR as the ratio of total positive
FFN (retrieval) DLA to total negative Attention (inhibition) DLA:
\begin{equation}
\mathrm{SGR}
...
\label{eq:sgr}
\end{equation}
where $[\cdot]_+$ retains only positive (retrieval) contributions from FFNs
and $[\cdot]_-$ retains only negative (inhibitory) contributions from
attention heads.  When $\mathrm{SGR} > 1$ retrieval overwhelms inhibition
and the model is predicted to hallucinate; when $\mathrm{SGR} < 1$
inhibition wins and negation succeeds.
...
\texttt{run\_pipeline.py}   & Orchestrator \\
\bottomrule
\end{tabular}
...
Negation failure rate  & 11.4\% & 12.4\% \\
Mean SGR (finite)      & 244.6  & 23.3   \\
Median SGR             & 27.1   & 7.7    \\
SGR\,$>$\,1 rate (all) & 58.0\% & 99.3\% \\
\bottomrule
\end{tabular}
\caption{Behavioural summary across models.}
\label{tab:results}
\end{table}

GPT-2 Small exhibits an 11.4\% negation failure rate, and Pythia-160M a
slightly higher 12.4\% (z\,=\,$-$3.27, p\,=\,0.001).  GPT-2's mean SGR
of 244.6 is dominated by tail cases with very weak inhibition; the median
(27.1) gives a more representative picture.  Pythia-160M has a much lower
mean SGR of 23.3 (median 7.7), indicating its inhibition circuit is
generally stronger in absolute terms, even though its failure rate is
marginally higher.

\subsection{SGR Distribution}
Figure~\ref{fig:sgrhist} shows the SGR distribution, colour-coded by
whether negation failed.  Both distributions are heavily right-skewed.
For GPT-2 Small, 58.0\% of all samples have SGR\,$>$\,1, while for
Pythia-160M the figure rises to 99.3\%.  Importantly, SGR\,$>$\,1 is a
\emph{necessary} but not \emph{sufficient} condition for negation failure:
many prompts with SGR\,$>$\,1 are still suppressed correctly, suggesting
that additional circuit-level factors (e.g.\ the absolute magnitude of
inhibition, or the presence of multiple suppression heads acting in
concert) modulate the final outcome.  Among failure cases specifically,
99.1\% of GPT-2 failures and 100\% of Pythia failures have SGR\,$>$\,1,
strongly supporting the diagnostic value of the metric.
...
\begin{figure}[t]
  ...
  \caption{SGR distribution across CounterFact samples.  Hatched bars
           ...
           Note: the ``SGR\,$>$\,1 rate'' annotated in the figure refers
           to the fraction of \emph{failure} cases with SGR\,$>$\,1,
           not the overall population rate.}
  \label{fig:sgrhist}
\end{figure}
...
sweep shows that increasing the scale factor narrows the gap between
the positive and negated prompt logits: the logit on the negated prompt
does not fall uniformly, but the \emph{retrieval--inhibition gap}
($\mathrm{logit}_{\mathrm{neg}} - \mathrm{logit}_{\mathrm{pos}}$) closes
systematically as $\alpha$ increases from 0.5 to 4, indicating that
inhibition is gaining ground against retrieval.  On the right, the
dataset-level experiment confirms that the negation failure rate
drops from 12.0\% at $\alpha=1$ (baseline) to 6.0\% at $\alpha=4$,
providing causal evidence that the identified heads are genuine
Inhibition Heads.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{../figures/main_output/amplification_sweep.png}
  \includegraphics[width=\linewidth]{../figures/main_output/amplification_failure_rate.png}
  \caption{Top: target logit vs.\ inhibition head amplification scale
           (single prompt).
           Bottom: dataset-level negation failure rate vs.\ scale.}
  \label{fig:amp}
\end{figure}

\subsection{Cross-Model Comparison}
Figure~\ref{fig:modelcomp} compares SGR distributions and failure rates
between GPT-2 Small and Pythia-160M.  Despite having a substantially
lower mean SGR (23.3 vs.\ 244.6), Pythia-160M has a marginally
\emph{higher} failure rate (12.4\% vs.\ 11.4\%, z\,=\,$-$3.27,
p\,=\,0.001).  This reveals an important nuance: the absolute magnitude
of SGR is not the sole predictor of failure; the shape of the inhibition
distribution matters.  Pythia has 99.3\% of samples at SGR\,$>$\,1
vs.\ GPT-2's 58.0\%, meaning Pythia has very few samples where
inhibition convincingly dominates.  Both models show significant
Spearman correlation between SGR and failure (r\,=\,0.15 for GPT-2,
r\,=\,0.11 for Pythia, both p\,$<$\,$10^{-50}$), supporting the
Competitive Gating Hypothesis across architectures.
