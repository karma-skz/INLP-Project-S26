...
Extended amplification experiments demonstrate that scaling inhibition heads by $\alpha\!=\!4$ roughly halves the failure rate in GPT-2 Small (11.4\%~$\to$~5.5\%) and significantly reduces it in Pythia-160M (9.1\%~$\to$~3.8\%).
...
\paragraph{Scaling does not fix negation.} Negation failure rates range from 9.1\% (Pythia-160M) to 12.9\% (GPT-2 Large). Remarkably, GPT-2 Large (774M parameters) has a \emph{higher} failure rate than GPT-2 Small (124M), confirming prior behavioural findings~\cite{truong-etal-2023-language} but now with a mechanistic explanation: larger FFN capacity increases retrieval strength without proportionally strengthening inhibition---the memory muscle grows faster than the logic brake.
...
\bottomrule
\end{tabular}
\caption{Amplification effectiveness on a 1,000-sample subset. Failure rate reduction at $\alpha\!=\!4$ vs.\ baseline. GPT-2 Large is the exception (\S\ref{sec:neg_results}).}
\label{tab:amp}
\end{table}
...
\item \textbf{SGR\,$<$\,1 rarity.} As noted in Section~\ref{sec:sgrlt1}, the perfect success rate for SGR\,$<$\,1 is based on very few samples due to the architectural dominance of the retrieval circuit. Validating this boundary condition on larger models remains future work.
...
\begin{itemize}
\item \textbf{Soft negation at head-level:} Perform head-level DLA for ``is unlikely to be'' and ``rarely'' to determine if soft negators recruit the same inhibition circuit as hard negation.
