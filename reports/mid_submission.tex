...
paradoxically increases its activation \cite{vrabcova2024}.  However, we
...
Geva et al.\ \shortcite{geva2021} and Dai et al.\ \shortcite{dai2022}
...
Wang et al.\ \shortcite{wang2023} established the framework for identifying
\emph{circuits}---subsets of model components responsible for specific
behaviours.  Hanna et al.\ \shortcite{hanna2024} identified
...
Truong et al.\ \shortcite{truong2023} showed that scaling up model size
does not fix negation failures.  Vrabcov\'{a} et al.\ \shortcite{vrabcova2024}
...
is that FFNs behave like key--value memory stores \cite{geva2021}: mid-layer
...
(12 layers, 12 heads, 768-dim) using TransformerLens \cite{nanda2022}.
We use the \textbf{CounterFact} dataset \cite{meng2022}, a benchmark of
...
\midrule
Residual stream decomposition       & \checkmark & \verb|run_benchmark.py| \\
Direct Logit Attribution (DLA)      & \checkmark & \verb|run_benchmark.py| \\
Per-layer FFN vs.\ Attn DLA         & \checkmark & \verb|run_benchmark.py| \\
Signal-to-Gate Ratio (SGR)          & \checkmark & \verb|run_benchmark.py| \\
Crossover Point detection           & \checkmark & \verb|run_benchmark.py| \\
Per-head DLA decomposition          & \checkmark & \verb|per_head.py| \\
Inhibition Head identification      & \checkmark & \verb|per_head.py| \\
Activation patching (causal)        & \checkmark & \verb|transformerLenstest.py| \\
Artificial amplification            & \checkmark & \verb|amplification.py| \\
CounterFact dataset integration     & \checkmark & \verb|load_dataset.py| \\
GPT-2 Small support                 & \checkmark & \verb|load_models.py| \\
Pythia-160M support                 & \checkmark & \verb|load_models.py| \\
Statistical tests (Spearman, CI)    & \checkmark & \verb|metrics.py| \\
Hard negation (``is not'')          & \checkmark & \verb|load_dataset.py| \\
